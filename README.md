...
| `token` | Required. A `Laika\Model\Model` subclass. `validateToken()` loads the user with its `find()`. |
...
`Laika\Auth\Exceptions\AuthException` is thrown for a misconfigured guard, such as a token provider that isn't a `Model`. Failed validation doesn't throw; it returns `null`.
